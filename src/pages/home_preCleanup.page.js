...
const HomePage = (params) => {
    ...
    const getWeatherComponent = (rootComponent) => {
        // console.log('============> rootComponent.options:', rootComponent)
        // console.log('============> weatherComponents:', weatherComponents)
        ...
        if (value === "show") {
            return (
                <div className="button-component-container"> 
                    <div className="button-component" onClick={(variable === 'show_image' && toggleShowCity) || (variable === 'show_weather' && toggleShowWeather)}>
                        <div className="button-component-text" onClick={(variable === 'show_image' && toggleShowCity) || (variable === 'show_weather' && toggleShowWeather)}>Hide</div> 
                        <VisibilityOffOutlinedIcon className="button-component-icon" onClick={(variable === 'show_image' && toggleShowCity) || (variable === 'show_weather' && toggleShowWeather)} />
                    </div>
                </div>
            )
        } else if (value === "hide") {
            return (
                <div className="button-component-container"> 
                    <div className="button-component" onClick={(variable === 'show_image' && toggleShowCity) || (variable === 'show_weather' && toggleShowWeather)}>
                        <div className="button-component-text" onClick={(variable === 'show_image' && toggleShowCity) || (variable === 'show_weather' && toggleShowWeather)}>Show</div> 
                        <VisibilityOutlinedIcon className="button-component-icon" onClick={(variable === 'show_image' && toggleShowCity) || (variable === 'show_weather' && toggleShowWeather)} />
                    </div>
        ...
        if (true) {
            if (rootComponent.type && !rootComponent.children) {
                if (rootComponent.type === 'button' && rootComponent.options.variable === 'show_weather' && rootComponent.options.value === 'show' && showWeather === 'show') {
                    ...
                    return (
                        <div className="button-component-container">
                        <div className="button-component" style={{ marginBottom: '20px' }} key={uuid()} id={location} onClick={(e) => selectLocation(e)}>
    ...
    const getLocationSelectComponent = (location) => {    
        const locationName = locationNames[location] && locationNames[location].name;
        // console.log('^^^^^^^^^ getLocationSelectComponent location:', location, locationName, locationNames)
        return (
                ...
                if (index === 0) {
                    return <div key={uuid()}>
                        ...
                        {list.components.map((componentId) => {
                            ...
                            // console.log('-----> rootComponent.type:', rootComponent.type, rootComponent.children, rootComponent.children.length)
                            if (rootComponent.type === 'condition') {
                                const subList = lists[rootComponent.children].components;
                                // console.log('=====> location, condition rootComponent, rootComponent.children, subList:', location, rootComponent, rootComponent.children, subList)
                                // render only the list from the selected location
                                if (location === rootComponent.options.value) {
                                    ...
                                    return <div>
                                        <div key={uuid()}>{getComponentById(subList[0])}</div>
                                        </div>
                                }
...
// (rootComponent.type === 'condition' && rootComponent.children.length !== undefined) {
//     rootComponent.children.map((componentId) => {
//         console.log('=====> from condition children.length !== undefined componentId:', componentId)
//         return <div key={uuid()}>{getComponentById(componentId)}</div>
//     })
// } else
...
// }
   ...
   // const getShowButtonComponent = () => {
    ...
    // else if (false && rootComponent.type && rootComponent.children) {
    ...
    // console.log('----> renderComponentArr.length:', renderComponentArr.length)
