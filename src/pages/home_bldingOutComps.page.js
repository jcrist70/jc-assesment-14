...
const HomePage = (params) => {
    ...
    const getWeatherComponent = (location) => {
    ...
    const getComponentById = (componentId) => {
        ...
        return (
            <h2>{rootComponent.type}:{rootComponent.children}:{rootComponent.options.variable}:{rootComponent.options.value}::{JSON.stringify(renderComponentArr.components)}</h2>
        )
    }
    const getLocationSelectComponent = (location) => {
        const locationName = locationNames[location].name;
        return (
    ...
    const getLocationImageComponent = (location) => {
        ...
        const locationName = locationNames[location].name;
        const src = locationNames[location].src;
